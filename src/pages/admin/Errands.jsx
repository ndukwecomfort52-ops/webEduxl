...
import ClanErrandsScreen from "./ClanErrandsScreen";
...
const Errands = () => {
  ...
  const { mutate: UpdateRunner, isLoading: isUpdateRunner } = useMutateData(
    "runners", // This is the endpoint segment for your POST request, e.g., /api/runners
    "PATCH"
  );
  ...
  const handleCreateRunner = async (e) => {
    ...
    let data = {
      clanEmail: newRunnerData.clanemail,
  ...
  const renderRunnerCard = (runner) => {
    // Get the primary clan (first active clan or first in array)
    const primaryClan =
      runner.clansData?.find((c) => c.status === "active") ||
      runner.clansData?.[0];
    const clan = primaryClan?.clan || runner.clan; // Fallback to direct clan reference

    return (
      <div
        key={runner._id}
        className="bg-blue-50 p-4 rounded-lg shadow-sm mb-3 border border-blue-200 hover:shadow-md transition-shadow"
      >
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-700 text-sm mb-1">
              ...
              {runner.user?.email || "N/A"}
            </p>
            <p className="text-gray-700 text-sm mb-1">
              <span className="font-medium">Phone:</span>{" "}
              {runner.phoneNumber || "N/A"}
            </p>
          </div>
          ...
          <div className="flex space-x-2">
            ...
            <button
              onClick={() => onDelete(runner._id)}
              className="px-3 py-1 bg-red-100 text-red-800 rounded text-sm hover:bg-red-200"
            >
              Delete
            </button>
            <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">
              {runner.errands?.length || 0} errands
            </span>
          </div>
        </div>
        ...
        {clan && (
          <div className="mt-2 pt-2 border-t border-blue-100">
            <p className="text-gray-700 text-sm">
              <span className="font-medium">Clan:</span> {clan.name || "N/A"}
            </p>
            <p className="text-gray-700 text-sm">
              <span className="font-medium">Clan ID:</span>{" "}
              {clan.uniqueClanID || "N/A"}
            </p>
            <p className="text-gray-700 text-sm">
              <span className="font-medium">Status:</span>{" "}
              {primaryClan?.status || "N/A"}
            </p>
          </div>
        )}
        ...
        {isEditing && (
          <div className="mt-3 pt-3 border-t border-gray-200">
            <div className="flex items-center space-x-2">
              <input
                type="email"
                placeholder="New clan email"
                name="clanemail"
                value={newRunnerData.clanemail}
  ...
  // <ClanErrandsScreen errand={errand} />;
    ...
    <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        <div className="flex border-b border-gray-200 mb-6">
          ...
          <button
            onClick={() => setActiveTab("clanerrands")}
            className={`ml-4 px-4 py-2 text-lg font-medium ${
              activeTab === "clanerrands"
                ? "text-blue-600 border-b-2 border-blue-600"
                : "text-gray-500 hover:text-gray-700"
            }`}
          >
        ...
        {activeTab === "runners" && (
          <div>
            ...
            ) : allRunnersData?.errands?.length === 0 ? (
              <p className="text-gray-600 text-center text-lg">
                No runners found.
              </p>
            ) : (
              <div>{allRunnersData?.errands?.map(renderRunnerCard)}</div>
            )}
          </div>
        )}
        ...
        {activeTab === "clanerrands" && (
          <div>
            {isLoadingErrands ? (
              ...
              <div>
                <ClanErrandsScreen errands={allErrandsData?.errands} />
              </div>
            )}
          </div>
        )}
